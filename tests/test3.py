...
ff=[g*32 for  g in [1,2,4,8]]

pre=f'{ff[0]}_'
model = UNet(3,num_segmentations=2,features=ff).to(config['device'])
...
def train():
    for epoch in range(config['epochs']):
        ...
        for image, seg in loop:
            ...
            loss = loss_fn(pred, seg.squeeze(1).long())
        ...
        ima.saveNumpy(preds[0,1].cpu().detach().numpy().astype(np.float32),f'/data/garbage/{pre}s{epoch}.nii.gz')
