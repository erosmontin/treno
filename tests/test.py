...
config = {
    ...
    "image_paths": "/data/MYDATA/DL_train_DS/birds/CUB_200_2011/images.txt",
    "epochs": 10,
...
model = UNet(3,features=[8, 16,32,64]).to(config['device'])
...
def train():
    for epoch in range(config['epochs']):
        ...
        for image, seg in loop:
            ...
            scaler.update()
            ima.saveNumpy(pred[0,0].cpu().detach().numpy().astype(np.float32),f'/data/garbage/s{epoch}.nii.gz')
